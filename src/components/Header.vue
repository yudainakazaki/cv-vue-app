    ...
    <header class="header">
        <div v-show="!mobile" class="header__nav">
            <nav v-for="(item, i) in routes" :key="i" class="header__nav__item">
                <RouterLink :to="{ path: item.path }" class="header__nav__item__label link" :title="item.name" @click="scrollToTop">
                    {{ item.name }}
                </RouterLink>
            </nav>
            <label class="header__nav__item">
                <a href="https://flowcv.com/resume/a0uwwj2sc9" target="_blank" class="header__nav__item__label link" title="CV">CV</a>
            </label>
        </div>
        <Transition name="fade" mode="out-in">
            <div 
                v-show="mobileNav" class="nav-wrapper"
                @touchmove.prevent>
                <div class="mobile-nav" @click="toggleMobileNav">
                    <nav v-for="(item, i) in routes" :key="i" class="mobile-nav__item">
                        <RouterLink :to="{ path: item.path }" class="mobile-nav__item__label" :title="item.name" @click="scrollToTop">
                            {{ item.name }}
                        </RouterLink>
                    </nav>
        ...
        <div class="menu-icon">
            <span 
                ...
                class="bx bx-chevrons-left menu-icon__icon"
                :class="{'menu-icon__icon--active': mobileNav}"
            ></span>
        </div>
    </header>
</template>
